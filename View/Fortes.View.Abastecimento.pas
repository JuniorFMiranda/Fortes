...
type
  ...
  private
    ...
    function ValidarSeNaoHaRegistroInserido(Silent: Boolean = False): Boolean;
...
begin
  if ValidarSeNaoHaRegistroInserido then
    Exit;
...
function TAbastecimentoForm.ValidarSeNaoHaRegistroInserido(Silent: Boolean = False): Boolean;
begin
  Result := TotalDeRegistrosLancados = 0;
  if Result then
  begin
    if not Silent then
     ShowMessage('Informe ao menos um combustível para finalizar esse abastecimento.');
  end;
end;
